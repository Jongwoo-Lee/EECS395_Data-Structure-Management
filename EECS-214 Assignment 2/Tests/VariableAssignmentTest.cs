...
namespace DictionaryTests
{
    ...
    public class VariableAssignmentTest
    {
        ...
        public void VariableAssignTest()
        {
            ...
            expression1.Run(dict);

            //test to see that the variable has been stored
            object result = dict.Lookup("x");
            Assert.AreEqual(result, 4, "Variable assignment failed; expected 4, got "+result);
        }
        ...
        public void VariableReassignmentTest()
        {
            ...
            //evaluate SyntaxTrees
            expression1.Run(dict);
            expression2.Run(dict);

            //test to see that the variable has been reassigned
            object result = dict.Lookup("x");
            Assert.AreEqual(result, 5, "VariableAssignment doesn't update existing variable; expected 5, got "+result);
        }
        ...
        public void IndirectAssignmentTest()
        {
            ...
            Assert.AreEqual(result, 4, "Variable incorrectly updated by assignment statement; expected 4, got "+result);
        }
    }
}
