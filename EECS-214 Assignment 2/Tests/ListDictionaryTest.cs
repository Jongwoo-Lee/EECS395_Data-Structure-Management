...
namespace DictionaryTests
{
    ...
    public class ListDictionaryTest
    {
        ...
        {
            var target = new ListDictionary(); // TODO: Initialize to an appropriate value
            target.Store("test", 1);
        ...
        {
            var target = new ListDictionary(); // TODO: Initialize to an appropriate value
            var testData = new object[] { "a", "b", "c", "d", "e", "f", "g", "h", "i", "j", 1, 2, 3, 4 };
            for (int i = 0; i < testData.Length; i++)
            {
                target.Store("x"+i, testData[i]);
            }
            for (int i = 0; i < testData.Length; i++)
            {
                var testValue = target.Lookup("x"+i);
